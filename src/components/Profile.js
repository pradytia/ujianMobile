...
class Profile extends Component {
    ...
    renderListPost = () => {
        ...
        return this.props.listPost.map((val,idx) => {
            ...
            return(
                <View style={styleObj}>
                    <TouchableWithoutFeedback onPress={()=> this.props.selectProfilePost(val)}>
                        <Image source={{uri : val.imageURL}} style={{ height : 125, width : '100%' }}/>
...
const mapStateToProps = ({ auth, post }) => {
    //proteksi
    var user = auth.user ? auth.user : { displayName: '', photoURL: '', uid : '' }
    var listPost = post.postList.filter((val,idx) => {
